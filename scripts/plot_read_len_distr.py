...
# def plot_read_len_distr(tsv_file, output_dir):
#     # Ensure the output directory exists
#     os.makedirs(output_dir, exist_ok=True)
    
#     # Read the TSV file into a Dask DataFrame
#     df = dd.read_csv(tsv_file, sep='\t', dtype={'ReadName': 'object', 'PKL_File': 'object', 'Index': 'int64', 'read_length': 'float64'})
    
#     # Extract the read lengths
#     read_lengths = df['read_length'].compute()
    
#     # Log-transform the read lengths for better visualization
#     log_read_lengths = np.log10(read_lengths[read_lengths > 0])
    
#     # Plot the read length distribution
#     plt.figure(figsize=(10, 6))
#     plt.hist(log_read_lengths, bins=100, color='skyblue', edgecolor='skyblue', alpha=0.7)
...
#     # Read only the 'ReadLength' column from all Parquet files into a Dask DataFrame
#     df = dd.read_parquet(parquet_files, columns=['ReadLength'])
    
#     # Extract the read lengths
#     read_lengths = df['ReadLength'].compute()
