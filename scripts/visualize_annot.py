...
def visualize_sequence_annotations(
    ...
    if len(read) != len(predicted_labels):
        print(
            f"Error: Length mismatch between read and predicted_labels for {read_name}. Skipping this read."
        )
        return []

    num_chunks = int(np.ceil(len(read) / chars_per_line))
    read_chunks = [
        read[i * chars_per_line : (i + 1) * chars_per_line] for i in range(num_chunks)
    ]
    label_chunks = [
        predicted_labels[i * chars_per_line : (i + 1) * chars_per_line]
    ...
    for page_start in range(0, num_chunks, max_chunks_per_page):
        ...
        fig_height = (
            header_height + num_page_chunks * chunk_height
        ...
        for ax_idx, (read_chunk, label_chunk) in enumerate(
            zip(page_read_chunks, page_label_chunks), start=1
        ):
            ax = axs[ax_idx]
            start_idx = 0
            ...
            for idx, (base, label) in enumerate(zip(read_chunk, label_chunk)):
                ...
                if (
                    current_label != label or idx == len(read_chunk) - 1
                ):  # End of section or end of chunk
                    if current_label:
                        label_position = start_idx / chars_per_line + (
                            idx - start_idx
                        ) / (2 * chars_per_line)
                        ax.text(
                            label_position,
...
def save_plots_to_pdf(
    ...
    with PdfPages(filename) as pdf:
        for sequence, annotated_read, read_name in zip(
            sequences, annotated_reads, read_names
        ):
            ...
            for label, regions in annotated_read.items():
                ...
                for start, end in zip(regions["Starts"], regions["Ends"]):
                    if start >= len(sequence) or end > len(
                        sequence
                    ):  # Check for out-of-bound regions
                        print(
