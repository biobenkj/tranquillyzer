...
# def visualize_sequence_annotations(colors, read_name, read, predicted_labels, architecture, reason, chars_per_line=100, header_max_length=100):
...
#     # Adjust the figure size dynamically based on the number of lines in the header
#     fig, axs = plt.subplots(num_chunks + 1, 1, figsize=(15, 1.2 * (num_chunks + 1 + header_lines - 1)), 
#                             gridspec_kw={'height_ratios': [0.2 * header_lines] + [1] * num_chunks}, dpi=300)

#     for ax in axs:
#         ax.axis('off')

#     # Display read name, architecture, and reason at the top, breaking into multiple lines if needed
...
#             if label:
#                 ax.text(idx / chars_per_line, 1, base, ha='center', va='center', color=colors[label], fontsize=10, fontweight='medium')
...
#                     ax.text(label_position, 0.5, current_label, ha='center', va='center', color=colors[current_label], fontsize=10)
#                 start_idx = idx
#                 current_label = label

#     plt.subplots_adjust(hspace=0.05)  # Reduce the space between lines
#     plt.tight_layout()
#     return fig, axs

# def save_plots_to_pdf(sequences, annotated_reads, read_names, filename, colors, chars_per_line=100):
...
#                 if label in ['architecture', 'reason']:
#                     continue
...
#             fig, axs = visualize_sequence_annotations(colors, read_name, sequence, predicted_labels, architecture, reason, chars_per_line)
#             pdf.savefig(fig)
#             plt.close(fig)
...
# def visualize_sequence_annotations(colors, read_name, read, predicted_labels, architecture, reason, chars_per_line=100, header_max_length=100):
#     num_chunks = int(np.ceil(len(read) / chars_per_line))  # Number of chunks based on chars_per_line
#     read_chunks = [read[i * chars_per_line: (i + 1) * chars_per_line] for i in range(num_chunks)]
#     label_chunks = [predicted_labels[i * chars_per_line: (i + 1) * chars_per_line] for i in range(num_chunks)]
...
#     fig_height = header_height + num_chunks * chunk_height  # Dynamically adjust the figure height
#     fig, axs = plt.subplots(num_chunks + 1, 1, figsize=(15, fig_height), 
#                             gridspec_kw={'height_ratios': [header_height] + [1] * num_chunks}, dpi=300)
