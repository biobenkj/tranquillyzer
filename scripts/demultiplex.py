...
def generate_demux_stats_pdf(
    pdf_output_file, match_tsv_file, cell_tsv_file, match_type_counter, cell_id_counter
):
    with PdfPages(pdf_output_file) as pdf:

        # Save match statistics to TSV
        match_df = pd.DataFrame(
            match_type_counter.items(), columns=["Match_Type", "Read_Count"]
        )
        ...
        if cell_id_counter:
            cell_id_data = {
                int(k): v for k, v in cell_id_counter.items() if k != "ambiguous"
            }
            cell_df = pd.DataFrame(
                cell_id_data.items(), columns=["Cell_ID", "Read_Count"]
            )
        ...
        # Match Type Bar Plot
        predefined_order = [
            "Exact match (i5 + i7 + CBC)",
            "Two out of three match",
            "Only CBC match",
            "Ambiguous",
        ]
        match_categories = [
            category for category in predefined_order if category in match_type_counter
        ]
        ...
        if cell_id_counter:
            ...
            plt.axvline(
                x=read_counts[int(len(read_counts) * 0.05)],
                color="r",
                linestyle="--",
                label="Elbow Point",
            )
