...
structural elements (e.g., adapters, barcodes, UMIs, polyA/T tails, cDNA regions). The first column defines the model
name, the second column defines the label sequence (using predefined labels like ADAPTER, CBC, UMI, POLYA, POLYT, cDNA),
and the third column provides the exact sequences for fixed elements. Variable elements like CBCs and UMIs are
represented by `N`'s, with their lengths determined by the number of `N`'s (e.g., `7N` is a random sequence with a
length of 7 bp). An example schema file can be found on
[GitHub](https://github.com/huishenlab/tranquillyzer/blob/main/utils/training_seq_orders.tsv).
